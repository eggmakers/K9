...
inline bool BUT_IIR_calc_freq_odd( double k[2] , double sample_freq, const double cutoff_freq )
{	
	if ( (cutoff_freq <= 0.0001) || (sample_freq <= 1.99 * cutoff_freq) ) {
			// no filtering
		return false;
	}
	double C1=tan(Pi*cutoff_freq/sample_freq);
...
/*二阶ButterWorth低通*/
	class Filter_Butter2_LP
	{
		...
		public:
			...
			//滤波
			inline double get_result() const {return this->out_1[0];}
...
/*四阶ButterWorth低通*/
	class Filter_Butter4_LP
	{
		...
		public:
			...
			//滤波
			inline double get_result() const {return this->out_2[0];}
			inline double run( double newdata )
			{
				if( this->available )
				{
...
/*八阶ButterWorth低通*/
	class Filter_Butter8_LP
	{
		...
		public:
			...
			//滤波
			inline double get_result() const {return this->out_4[0];}
			inline double run( double newdata )
			{
				if( this->available )
				{
...
/*可变阶数ButterWorth低通(偶数阶 最高8阶)*/
	class Filter_Butter_LP
	{
		...
		public:
			...
			inline bool set_cutoff_frequency( uint8_t order, double FS , double FC )
			{
				...
				{	//奇数阶
					...
					for( uint8_t i=0; i<order/2; ++i ) {
						available &= BUT_IIR_calc_freq( this->k[i] , FS , FC , (1.0+i) / order );
					}
					available &= BUT_IIR_calc_freq_odd( this->k[order/2], FS, FC );
				...
				{	//偶数阶
					...
					uint8_t n = order*2;
					for( uint8_t i=0; i<order/2; ++i ) {
						available &= BUT_IIR_calc_freq( this->k[i] , FS , FC , (1.0+i*2) / n );
					}
					this->available = available;
					return available;
				}
			...
			{
				if( order & 1 )	//奇数阶
					return this->out[order/2][0];
				else
					return this->out[order/2-1][0];
			}
			inline double run( double newdata )
			{
				if( this->available )
				{
					...
					{	//奇数阶
						...
						this->out[n][0] = this->k[n][0]*( newdata + this->in[n][0] ) - this->k[n][1]*this->out[n][0];
						this->in[n][0] = newdata;
						newdata = this->out[n][0];
					}
					else
					{	//偶数阶
						uint8_t n = order/2;
