...
float adcGet_MainBaterry1_Current()
{
	...
	if( ReadParam("Bat_CurrentMKp", 0, 0, (uint64_t*)kp, 0 ) == PR_OK )
		return kp[0] * ((float)ADC3ConvertedData[4] * 3.3f / 65535.0f );
	else
		return 0;
}
...
float adcGet_MainBaterry2_Voltage()
{
	float kp[2];
	if( ReadParam("Bat2_VoltMKp", 0, 0, (uint64_t*)kp, 0 ) == PR_OK )
...
{
	return (110.0f - 30.0f) / (TS_CAL2 - TS_CAL1) * ( (float)ADC3ConvertedData[5] - TS_CAL1 ) + 30.0f;
}
...
/*
	...
	Battery1_ADC -- PC0 ADC3_INP10
	Battery2_ADC -- PC1 ADC3_INP11
	舵机电压_ADC -- PC2 ADC3_INP0
	Current_ADC -- PC3 ADC3_INP1
	TEMP_ADC -- ADC3_INP18*/
	
//配置引脚	
	RCC->AHB4ENR |= (1<<2) ;
	delay(1e-2);
	set_register( GPIOC->MODER , 0b11 , 0*2 , 2 );	//PC0
	set_register( GPIOC->MODER , 0b11 , 1*2 , 2 );	//PC1	
	set_register( GPIOC->MODER , 0b11 , 2*2 , 2 );	//PC2	
	set_register( GPIOC->MODER , 0b11 , 3*2 , 2 );	//PC3
  ...
  while((ADC3->CR &(1<<31))==(1<<31)){};	
	...
	ADC3->SMPR1 = (0b111<<0) | (0b111<<3);
	ADC3->PCSEL = (1<<0) | (1<<1) | (1<<10) | (1<<11) | (1<<18) | (1<<19);
	ADC3->SQR1 = (19<<6) | (10<<12) | (11<<18) | (0<<24) | ((ADC_CHANNELS-1)<<0);
	ADC3->SQR2 = (1<<0) | (18<<6);
