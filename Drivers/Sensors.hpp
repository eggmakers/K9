...
// 透传
#define PassThrough_Sensors_Count 5 // 透传传感器最大数量
...
extern const uint8_t Slam_Magnetometer_Index;
extern const uint8_t Ins_Magnetometer_Index;
extern const uint8_t Internal_Magnetometer_Index;
struct IMU_Sensor
{
	bool slam;			   // 是否slam传感器(仅罗盘有)
...
extern const uint8_t default_insAlign_sensor_index;
...
enum Position_Sensor_DataType
{
	...
	// 不进行姿态解算补偿的传感器数据
	Position_Sensor_DataType_s_xy_nAC = 100,
	Position_Sensor_DataType_s_z_nAC = 101,
	Position_Sensor_DataType_s_xyz_nAC = 102,

	Position_Sensor_DataType_v_xy_nAC = 108,
	Position_Sensor_DataType_v_z_nAC = 109,
	Position_Sensor_DataType_v_xyz_nAC = 110,
...
{
	Position_Sensor_frame_ENU = 0,		   // 位置速度数据在ENU坐标系下
	Position_Sensor_frame_BodyHeading = 1, // 位置ENU，速度数据x为机头朝向（与地面平行），y为朝向机头左方（与地面平行），z为上方
...
struct Position_Sensor_Data
{
	...
	double delay;					   // 传感器延时
	double xy_trustD;				   // 信任度cm 0最高信任度
	double xy_LTtrustD;
	double z_trustD; // 信任度cm 0最高信任度
	double z_LTtrustD;
	double sample_time; // 采样时间

	Position_Sensor_Type sensor_type;		  // 传感器类型（见枚举注释）
	Position_Sensor_DataType sensor_DataType; // 传感器数据类型（见枚举注释）
	Position_Sensor_frame data_frame;		  // 速度数据坐标系（见枚举注释）
