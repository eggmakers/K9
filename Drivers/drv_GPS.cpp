...
#define write_ubx(buf, i, x, cka, ckb) \
	{                                  \
		buf[i++] = x;                  \
		cka += x;                      \
		ckb += cka;                    \
...
GPS_CheckBaud:
	while (1)
	{
		...
		// 发送配置
		driver_info.port.write(ubloxClr, sizeof(ubloxClr), -1, -1);
		...
		for (uint32_t i = 0; i < sizeof(ubloxInit); i++)
		{
			if (i < sizeof(ubloxInit) - 1 && ubloxInit[i] == 0xB5 && ubloxInit[i + 1] == 0x62 && i > 2)
			{
				...
				os_delay(0.15);
				driver_info.port.write(&ubloxInit[i - sd_length], sd_length, -1, -1);
				sd_length = 0;
			}
			++sd_length;
		}
		driver_info.port.wait_sent(1);
		os_delay(0.15);
		driver_info.port.write(&ubloxInit[sizeof(ubloxInit) - sd_length], sd_length, -1, -1);
		...
		{ // 发送GNSS配置
			...
			for (uint8_t i = 0; i < sizeof(GNSS_cfg_header); ++i)
					...
					for (uint8_t i = 0; i < 7; ++i)
				{
					...
					{
						if (k != 4)
							write_ubx(buf, ind, GNSS_cfgs[i][k], cka, ckb) else
							{
								if (gps_cfg.GNSS_Mode[0] & (1 << i))
									write_ubx(buf, ind, 1, cka, ckb) else write_ubx(buf, ind, 0, cka, ckb)
							}
					}
				}
