...
static const uint8_t ubloxInit[] = {
	...
	0xB5, 0x62, 0x06, 0x08, 0x06, 0x00, 0xe8, 0x03, 0x01, 0x00, 0x01, 0x00, 0x01, 0x39, // set rate to 1Hz
...
static inline uint16_t GPS_ParseByte(GPS_State_Machine *state_machine, uint8_t *frame_datas, uint8_t r_data)
{
	...
	case 1: // 读Class ID和包长度
	{
		...
		if (state_machine->frame_datas_ind == 6)
		{
			...
			if (state_machine->frame_datas_length > 4 && state_machine->frame_datas_length < 2048)
		...
		if (driver_info.port.read(&r_data, 1, 2, 0.1))
		{
			...
			if (pack_length)
			{
				...
				{ // UBX-NAV-RELPOSNED
					...
					debug_test[25] = pack->relPosE;
					debug_test[26] = pack->relPosN;
					debug_test[27] = -pack->relPosD;
					debug_test[28] = pack->accN;
				}
			}
...
static bool RTK_DriverInit(Port port, uint32_t param)
{
	...
	xTaskCreate(RTK_Server, "RTK", 3000, driver_info, SysPriority_ExtSensor, NULL);
	return true;
}

void init_drv_RTK_Track()
{
	PortFunc_Register(19, RTK_DriverInit);
}
