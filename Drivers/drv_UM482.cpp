...
static bool send_init_msg(DriverInfo &driver_info)
{
	int len = 0;
	char UM482Clr[100];
	len = sprintf(UM482Clr, "UNLOG COM2\r\n");
	driver_info.port.write((uint8_t *)&UM482Clr[0], len, portMAX_DELAY, portMAX_DELAY);
	driver_info.port.wait_sent(1);
	driver_info.port.reset_rx(0.1);
	len = sprintf(UM482Clr, "CONFIG\r\n");
	driver_info.port.write((uint8_t *)&UM482Clr[0], len, portMAX_DELAY, portMAX_DELAY);
	driver_info.port.wait_sent(1);
	os_delay(0.15);
	...
	{
		if (driver_info.port.read((uint8_t *)&UM482Clr[0], 1, 0.5, 1) == 0)
			return false;
		if (len < CmdResultLen)
		{
			if (UM482Clr[0] == CmdResult[len])
				++len;
			else
				len = 0;
		}
	...
	for (uint8_t i = 0; i < 22; i++)
	{
		if (i == 0) // 停止所有信息输出
			...
			len = sprintf(UM482Clr, "MODE ROVER\r\n");

		else if (i == 2) // 设置 RTK 模块指令，数据最大龄期，秒为单位
			len = sprintf(UM482Clr, "CONFIG RTK TIMEOUT 600\r\n");

		else if (i == 3) // 设置接收的 DGPS差分数据的最大龄期，秒为单位，接收到的滞后于指定龄期的 DGPS 差分数据被忽略，也用于禁止 DGPS 定位计算
			len = sprintf(UM482Clr, "CONFIG DGPS TIMEOUT 600\r\n");

		else if (i == 4) // 设置双天线接收机的主天线（ANT1）与从天线（ANT2）之间距离保持固定
			len = sprintf(UM482Clr, "CONFIG HEADING FIXLENGTH\r\n");

		else if (i == 5) // 设置 EVENT 功能，下降沿有效、两个有效脉冲之间的最短时间要求, 单位ms。若小于 TGuard，则第二个 Event 被忽视。默认值: 4，最小： 2；最大：3,599,999
			len = sprintf(UM482Clr, "CONFIG EVENT ENABLE NEGATIVE 2\r\n");

		else if (i == 6) // 设置关闭单频定位状态的判定条件，基线长度超过 5km 时，精度满足需求依然可以使用固定解
			len = sprintf(UM482Clr, "CONFIG SFRTK Enable\r\n");

		else if (i == 7) // 将 BDS 系统卫星的 B1C&B2a 信号编入 RTCM 协议
			len = sprintf(UM482Clr, "CONFIG RTCMB1CB2a Enable\r\n");

		else if (i == 8) // 使能接收机跟踪 GPS 卫星系
			len = sprintf(UM482Clr, "UNMASK GPS\r\n");

		else if (i == 9) // 使能接收机跟踪 BDS 卫星系统
			len = sprintf(UM482Clr, "UNMASK BDS\r\n");

		else if (i == 10) // 使能接收机跟踪 GLO 卫星系统
			len = sprintf(UM482Clr, "UNMASK GLO\r\n");

		else if (i == 11) // 使能接收机跟踪 GAL 卫星系统
			len = sprintf(UM482Clr, "UNMASK GAL\r\n");

		else if (i == 12) // 使能接收机跟踪 QZSS 卫星系统
			len = sprintf(UM482Clr, "UNMASK QZSS\r\n");

		else if (i == 13) // 输出 EVENT 发生时刻的精确绝对时间及相对时间
			len = sprintf(UM482Clr, "LOG EVENTMARKB ONCHANGED\r\n");

		else if (i == 14) // 设置接收机跟踪卫星的卫星截止角度
			len = sprintf(UM482Clr, "MASK 10\r\n");

		else if (i == 15) // 接收机默设置为动态模式
			len = sprintf(UM482Clr, "RTKDYNAMICS DYNAMIC\r\n");

		else if (i == 16) // 接收机运动的航向
			len = sprintf(UM482Clr, "LOG HEADINGB ONTIME 1\r\n");

		else if (i == 17) // UTC 时间
			len = sprintf(UM482Clr, "LOG TIMEB ONTIME 0.1\r\n");

		else if (i == 18) // 精度因子信息
			len = sprintf(UM482Clr, "LOG BESTVELB ONTIME 0.1\r\n");

		//		else if (i==19)// 原始观测数据信息
		//			len = sprintf( UM482Clr,  "LOG COM1 RANGEB ONTIME 0.1\r\n");
		//
		//		else if (i==20)// GLONASS 导航电文
		//			len = sprintf( UM482Clr,  "LOG COM1 GLORAWSTRINGB ONCHANGED\r\n");

		//		else if (i==21)// GPS 导航电文
		//			len = sprintf( UM482Clr,  "LOG COM1 RAWGPSSUBFRAMEB ONCHANGED\r\n");
		//
		//		else if (i==22)// BDS 导航电文
		//			len = sprintf( UM482Clr,  "LOG COM1 BDSRAWNAVSUBFRAMEB ONCHANGED\r\n");

		//		else if (i==23)// QZSS 导航电文
		//			len = sprintf( UM482Clr,  "LOG COM1 QZSSRAWSUBFRAMEB ONCHANGED\r\n");

		else if (i == 19) // 主天线计算出的最佳可用位置和速度Bestnav
			len = sprintf(UM482Clr, "LOG BESTPOSB ONTIME 0.1\r\n");

		//		else if (i==22)// 20Hz 的 GNGGA 信息
		//			len = sprintf( UM482Clr,  "GPGGA 0.05\r\n");

		//		else if (i==20)// HEADING2
		//			len = sprintf( UM482Clr,  "LOG HEADING2B ONCHANGED\r\n");
		//
		//		else if (i==21)// HEADING2
		//			len = sprintf( UM482Clr,  "HEADINGMODE VARIABLELENGTH\r\n");

		else if (i == 20) // 设置com2波特率为460800,8位数据位,无校验,一位停止位
			len = sprintf(UM482Clr, "CONFIG COM1 460800 8 n 1\r\n");
		else if (i == 21) // 设置com2波特率为460800,8位数据位,无校验,一位停止位
			len = sprintf(UM482Clr, "CONFIG COM2 460800 8 n 1\r\n");
	...
	while (1)
	{
		...
		while (RxChkStartTime.get_pass_time() < 3)
		{
			...
			if (driver_info.port.read(&r_data, 1, 0.5, 0.1))
			{
				...
				if (res)
				{
					if (gps_state.frame_id == 99)
					{
						bestvel_pack = *(BESTVEL_Pack *)&frame_datas[gps_state.header_length];
						velPackReceived = true;
					}
					if (gps_state.frame_id == 42 && velPackReceived)
					{ // 已识别,跳转到gps接收程序
						goto GPS_Present;
					}
				}
		...
		if (driver_info.port.read(&r_data, 1, 2, 0.1))
		{
			if (GPS_ParseByte(&gps_state, frame_datas, r_data))
			{
				...
				{ // BESTPOS 最佳位置
					...
					Best_Pos_Pack *pack = (Best_Pos_Pack *)&frame_datas[gps_state.header_length];
					...
					double hdop = 0.5 * safe_sqrt(sq(pack->LatAcc) + sq(pack->LonAcc));
					double vdop = 0.5 * pack->HeightAcc;
					...
					velocity.y = bestvel_pack.horSpeed * cos(degree2rad(bestvel_pack.horSpeedAngle)) * 100; // North
					velocity.x = bestvel_pack.horSpeed * sin(degree2rad(bestvel_pack.horSpeedAngle)) * 100; // East
					velocity.z = bestvel_pack.VerticalSpeed * 100;											// Up
				...
				else if (gps_state.frame_id == 99)
				{ //	BESTVEL 最佳速度
					last_update_time = TIME::now();
					bestvel_pack = *(BESTVEL_Pack *)&frame_datas[gps_state.header_length];
				}
				else if (gps_state.frame_id == 971)
				{ // UNIHEADING 航向信息
					last_update_time = TIME::now();
					struct Heading_Pack
					{
...
static bool RTK_DriverInit(Port port, uint32_t param)
{
	...
	xTaskCreate(RTK_Server, "RTK482", 2500, driver_info, SysPriority_ExtSensor, NULL);
	return true;
}

void init_drv_RTK_UM482()
{
	PortFunc_Register(21, RTK_DriverInit);
}
