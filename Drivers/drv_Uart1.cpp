...
static MessageBufferHandle_t TxStreamBuffer;
...
void init_drv_Uart1()
{
	...
	// 发送缓冲区
	TxStreamBuffer = xTxMessageBufferCreate(TxStreamBufferSize, 1);
	// 接收缓冲区
	...
	DMA1_Stream3->CR = (1 << 20) | (0 << 16) | (0 << 13) | (1 << 10) | (0 << 9) | (0b01 << 6) | (1 << 4);
	...
	UartPort.txSpacesAvailable = Uart1_TxSpacesAvailable;
	PortRegister(1, UartPort);
}

static void TxTCB(void *pvParameter1, uint32_t ulParameter2)
{
	uint16_t length = xMessageBufferReceive(TxStreamBuffer, TxBuffer, TxBufferSize, 0);
	if (length > 0)
...
uint16_t Write_Uart1(const uint8_t *data, uint16_t length, double Send_waitTime, double Sync_waitTime)
{
	...
	{
		uint16_t data_sent = xMessageBufferSend(TxStreamBuffer, data, length, Send_waitTicks);
		if (TxCompleted)
		{
			// USB空闲可发送
			uint16_t sd_length;
			sd_length = xMessageBufferReceive(TxStreamBuffer, TxBuffer, TxBufferSize, 0);
			if (sd_length > 0)
...
// 获取缓冲区剩余空间
uint16_t Uart1_TxSpacesAvailable()
{
	return xMessageBufferSpaceAvailable(TxStreamBuffer);
}
