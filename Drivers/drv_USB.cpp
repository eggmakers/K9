...
MessageBufferHandle_t usb_tx_streambufferhandle_t;

// USB_CDC读写缓冲区大小
#define USB_RX_BUFFER_LENGTH 1024 * 4
#define USB_TX_BUFFER_LENGTH 1024 * 4
...
uint16_t Write_USBD_VCOM(const uint8_t *data, uint16_t length, double Send_waitTime, double Sync_waitTime)
{
	...
	if (xSemaphoreTakeRecursive(USBD_VCOM_TxSemphr, Sync_waitTicks) == pdTRUE)
	{
		uint16_t data_sent = xMessageBufferSend(usb_tx_streambufferhandle_t, data, length, 0);
...
// 获取缓冲区剩余空间
uint16_t USBD_TxSpacesAvailable()
{
	return xMessageBufferSpaceAvailable(usb_tx_streambufferhandle_t);
}
...
void init_drv_USB()
{
	...
	// 读写缓冲区
	usb_rx_streambufferhandle_t = xStreamBufferCreate(USB_RX_BUFFER_LENGTH, 1);
	usb_tx_streambufferhandle_t = xTxMessageBufferCreate(USB_TX_BUFFER_LENGTH, 1);
	...
	USBDVCOM.txSpacesAvailable = USBD_TxSpacesAvailable;
