...
/*7位地址发送数据
	...
	Sync_waitTime：超时时间
*/
bool ExtIIC_SendAddr7( uint8_t addr, const uint8_t* datas, uint16_t length, IICSpeed speed, double Sync_waitTime )
{
	if( length == 0 )
		return false;
	...
	if( Lock_ExtIIC(Sync_waitTime) )
	{
		...
		if( speed == IICSpeed_Fast400k )
			I2C1->TIMINGR = TIM400k;	//400k
		else if( speed == IICSpeed_Standard50k )
			I2C1->TIMINGR = TIM50k;	//35k
		else
			I2C1->TIMINGR = TIM100k;	//100k
...
/*7位地址发送并接收数据
	...
	Sync_waitTime：超时时间
*/
bool ExtIIC_ReceiveAddr7( uint8_t addr, uint8_t* rx_datas, uint16_t rx_length, IICSpeed speed, double Sync_waitTime )
{
	if( rx_length==0 )
		return false;
	...
	if( Lock_ExtIIC(Sync_waitTime) )
	{
		...
		/*配置DMA*/
			...
			//设置DMA传输数量
			DMA1_Stream7->NDTR = RMRxLength + rx_length;
			//使能DMA
			DMA1_Stream7->CR |= (1<<0);	
		/*配置DMA*/		
...
/*7位地址发送并接收数据
	...
	Sync_waitTime：超时时间
*/
bool ExtIIC_SendReceiveAddr7( uint8_t addr, const uint8_t* tx_datas, uint16_t tx_length, uint8_t* rx_datas, uint16_t rx_length, IICSpeed speed, double Sync_waitTime )
{
...
void init_drv_ExtIIC()
{
	/*配置IIC1*/
		...
		//IIC时间配置
		#ifdef SYSFREQ480
			//I2C1->TIMINGR = 0xb03fdb;	//400k
			I2C1->TIMINGR = 0x307075B1;	//100k
		#else
			//I2C1->TIMINGR = 0x9034B6;//400K
			I2C1->TIMINGR = 0x10C0ECFF;//100K
		#endif
	...
	/*配置GPIO PB8(IIC1 SCL) PB9(IIC1 SDA)*/
		...
		//引脚开漏上拉
		set_register( GPIOB->OTYPER , 0 , 8*1 , 1 );	//PB8
		set_register( GPIOB->OTYPER , 1 , 9*1 , 1 );	//PB9
		set_register( GPIOB->PUPDR , 0b01 , 8*2 , 2 );	//PB8
		set_register( GPIOB->PUPDR , 0b01 , 9*2 , 2 );	//PB9
		//中速
		set_register( GPIOB->OSPEEDR , 0b10 , 8*2 , 2 );	//PB8
		set_register( GPIOB->OSPEEDR , 0b10 , 9*2 , 2 );	//PB9
		//复用IIC1
