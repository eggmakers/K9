...
public:
	...
	// 分解四元数
	inline Quaternion get_RP_quat(double *yaw = 0) const
	{
		double qw1 = safe_sqrt(sq(qw) + sq(qz));
		if (!is_zero(qw1))
		{
			double inv_qw1 = 1.0 / qw1;
			double qw2 = qw * inv_qw1;
			double qz2 = qz * inv_qw1;
			if (yaw)
			{
				*yaw = atan2(qz2, qw2) * 2;
			}

			double qx1 = qx * qw2 + qy * qz2;
			double qy1 = qy * qw2 - qx * qz2;
			return Quaternion(qw1, qx1, qy1, 0);
		}
		else
		{
	...
	*/
	...
	inline void rotate_delta_angle(vector3<double> delta_angle)
	{
		...
		double Gw = angle_cosin;
		double Gx = delta_angle.x * qxyz_scale;
		double Gy = delta_angle.y * qxyz_scale;
		double Gz = delta_angle.z * qxyz_scale;
		double tqw = qw;
		double tqx = qx;
		double tqy = qy;
		double tqz = qz;
		...
		//			gw*qx + gx*qw - gy*qz + gz*qy,
		//			gw*qy + gy*qw + gx*qz - gz*qx,
		//			gw*qz - gx*qy + gy*qx + gz*qw
		//			qw = Gw*tqw - Gx*tqx - Gy*tqy - Gz*tqz;
		//			qx = Gw*tqx + Gx*tqw - Gy*tqz + Gz*tqy;
		//			qy = Gw*tqy + Gy*tqw + Gx*tqz - Gz*tqx;
		//			qz = Gw*tqz - Gx*tqy + Gy*tqx + Gz*tqw;
...
public:
	...
	// 四元数分解
	inline Quaternion_Ef get_RP_quat(double *yaw = 0) const
	{
		double qw1 = safe_sqrt(sq(qw) + sq(qz));
		if (!is_zero(qw1))
		{
