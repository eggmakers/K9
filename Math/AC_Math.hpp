...
#define SSL_AIR_DENSITY 1.225
#define SSL_AIR_PRESSURE 101325.01576
#define SSL_AIR_TEMPERATURE 288.15
/*常量定义*/
...
// 移除-band至+band死区
template <class T>
inline T remove_deadband(T data, T band)
{
	if (data > band)
		return data - band;
	else if (data < -band)
...
//-max至+max范围限幅
template <class T>
inline T constrain(const T x, const T max)
{
	if (x < -max)
		return -max;
	else if (x > max)
