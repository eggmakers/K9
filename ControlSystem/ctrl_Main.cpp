...
/*电池信息*/
	...
	static Battery2Cfg bat2Cfg;
	static TD4_Lite RMPercentFilter1;
	static TD4_Lite RMPercentFilter2;
	...
	static inline float get_MainBatteryRMPercent( float volt, const Battery2Cfg* cfg )
	{
		if( volt > cfg->STVoltage[0] + cfg->VoltP10[0] )
			return 100;
		for( int8_t i = 10; i >= 1 ; --i )
...
static void ControlSystem_Task(void* pvParameters)
{
	...
	uint32_t bat2Id = batteryRegister(2);
	...
	/*电池1*/
	/*电池2*/
		ReadParamGroup( "Battery2", (uint64_t*)&bat2Cfg, 0 );
		batVolt = adcGet_MainBaterry2_Voltage();
		batVoltComp = batVolt - (getCtrlBatId()==2?getVoltKp():0)*thr;
		bat2Percent = get_MainBatteryRMPercent( batVoltComp, &bat2Cfg );
		batteryUpdate( 2, bat2Id,
										batVolt>5,	//available
										batVolt,	//totalVoltRaw
										batVoltComp, //totalVolt
										bat2Cfg.STVoltage[0],	//stVolt
										0,	//total current
										0,	//power usage
										bat2Cfg.Capacity[0], //capacity
										bat2Percent,	//percent
										-500,	//temperature
										UINT16_MAX, //cycle count
										0,	//error flags
										0, 0 );
		RMPercentFilter2.reset();
		RMPercentFilter2.x1 = bat2Percent;
	/*电池2*/
	...
	while(1)
	{
		...
		{	//400hz
			...
			/*电压测量滤波*/
				...
				if( ++VoltMeas_counter >= BatMonT*CtrlRateHz )
				{
					...
					/*电池2*/
						...
						batteryUpdate( 2, bat2Id,
														...
														0,	//power usage
														bat2Cfg.Capacity[0], //capacity
														RMPercentFilter2.get_x1(),	//percent
														-500,	//temperature
														UINT16_MAX, //cycle count
														0,	//error flags
														0, 0 );
				...
				RMPercentFilter2.track4( bat2Percent, 1.0/CtrlRateHz, RMPercentFilterP,RMPercentFilterP,RMPercentFilterP,RMPercentFilterP );
			/*电压测量滤波*/
		}
...
void init_ControlSystem()
{
	...
	/*注册电池参数*/
		...
		bat2Cfg.STVoltage[0] = 11.6;
		bat2Cfg.VoltMKp[0] = 38;
		bat2Cfg.Mode[0] = 0;
		bat2Cfg.Capacity[0] = 300;
		bat2Cfg.VoltP0[0] = -1.0;
		bat2Cfg.VoltP1[0] = -0.6;
		bat2Cfg.VoltP2[0] = -0.4;
		bat2Cfg.VoltP3[0] = -0.2;
		bat2Cfg.VoltP4[0] = -0.0;
		bat2Cfg.VoltP5[0] = +0.1;
		bat2Cfg.VoltP6[0] = +0.2;
		bat2Cfg.VoltP7[0] = +0.3;
		bat2Cfg.VoltP8[0] = +0.4;
		bat2Cfg.VoltP9[0] = +0.5;
		bat2Cfg.VoltP10[0] = +0.6;
		MAV_PARAM_TYPE param2_types[] = {
			MAV_PARAM_TYPE_REAL32 ,	//STVoltage
			MAV_PARAM_TYPE_REAL32 ,	//VoltMKp
			MAV_PARAM_TYPE_UINT32 ,	//Mode
			MAV_PARAM_TYPE_REAL32 ,	//Capacity
			MAV_PARAM_TYPE_REAL32 ,	//VoltP0
			MAV_PARAM_TYPE_REAL32 ,	//VoltP1
			MAV_PARAM_TYPE_REAL32 ,	//VoltP2
		...
		SName param2_names[] = {
			"Bat2_STVoltage" ,	//UAV Type
			"Bat2_VoltMKp" ,	//VoltMKp
			"Bat2_Mode" ,	//Mode
			"Bat2_Capacity" ,	//Capacity
			"Bat2_VoltP0" ,	//VoltP0
			"Bat2_VoltP1" ,	//VoltP1
			"Bat2_VoltP2" ,	//VoltP2
			"Bat2_VoltP3" ,	//VoltP3
			...
			"Bat2_VoltP8" ,	//VoltP8
			"Bat2_VoltP9" ,	//VoltP9
			"Bat2_VoltP10" ,	//VoltP10
		};
		ParamGroupRegister( "Battery2", 1, 15, param2_types, param2_names, (uint64_t*)&bat2Cfg );
	/*注册电池参数*/
}