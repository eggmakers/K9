...
#define reset_line_track_state(maxv, maxacc, desired_vel) \
	{                                                     \
...
void ctrl_Position()
{
	...
	is_Attitude_Control_Enabled(&Attitude_Control_Enabled);
	...
	vector3<double> AccelerationENU;
	get_AccelerationENU_Ctrl(&AccelerationENU);
	...
	{ // 水平位置控制
		...
		{ // 手动绕圈模式
			if (inFlight)
			{
				...
				TargetVelocity.x = T2r.x + TargetCircleVel.x;
				TargetVelocity.y = T2r.y + TargetCircleVel.y;
				TargetVelocity_1.x = T2r_1.x;
				TargetVelocity_1.y = T2r_1.y;
				TargetVelocity_2.x = T2r_2.x;
				TargetVelocity_2.y = T2r_2.y;
			}
			else
			{ // 没起飞前在绕圈模式
				// 回到position模式
		...
		{ // 悬停模式
			if (inFlight)
			{
				...
				TargetVelocity.x = T2.x;
				TargetVelocity.y = T2.y;
				TargetVelocity_1.x = T21P * T2_1.x;
				TargetVelocity_1.y = T21P * T2_1.y;
				TargetVelocity_2.x = T2_2.x;
				TargetVelocity_2.y = T2_2.y;
			}
			else
			{ // 没起飞前在位置控制模式
				// 重置期望位置
		...
		{ // OffBoard模式
			if (inFlight)
			{
				...
				{ // 控制位置+速度+加速度
					...
					TargetVelocity.x = T2.x + target_velocity.x;
					TargetVelocity.y = T2.y + target_velocity.y;
					TargetVelocity_1.x = T21P * T2_1.x + target_acc.x;
					TargetVelocity_1.y = T21P * T2_1.y + target_acc.y;
					TargetVelocity_2.x = T2_2.x;
					TargetVelocity_2.y = T2_2.y;
				}
				else
				{ // 速度+加速度
					TargetVelocity.x = target_velocity.x;
					TargetVelocity.y = target_velocity.y;
					TargetVelocity_1.x = T21P * target_acc.x;
					TargetVelocity_1.y = T21P * target_acc.y;
				}
			}
			else
			{ // 没起飞前在位置控制模式
		...
		case Position_ControlMode_RouteLine:
		{
			if (inFlight)
			{
				...
				TargetVelocity.x = T2r.x + T2d.x;
				TargetVelocity.y = T2r.y + T2d.y;
				TargetVelocity_1.x = T21P * T2r_1.x + T2d_1.x;
				TargetVelocity_1.y = T21P * T2r_1.y + T2d_1.y;
				TargetVelocity_2.x = T2r_2.x + T2d_2.x;
				TargetVelocity_2.y = T2r_2.y + T2d_2.y;
