...
/*机型控制方式*/
	...
	/*多旋翼*/
		...
		{
			//初始化姿态ESO
			((ESO_AngularRate*)CtrlM_ESO[0])->init( 
				(cfg.Mode[0] & ATTCTRL_MODE_RP_LEARN_DIS_BIT) ? false : false,
				cfg.order[0], cfg.T[0], cfg.b[0], cfg.beta[0], cfg.beta[0], 
				cfg.orderD[0], cfg.betaD[0],
				CtrlRateHz*CtrlRateDiv );
			((ESO_AngularRate*)CtrlM_ESO[1])->init( 
				(cfg.Mode[0] & ATTCTRL_MODE_RP_LEARN_DIS_BIT) ? false : false,
				cfg.order[0], cfg.T[0], cfg.b[2], cfg.beta[0], cfg.beta[0], 
				cfg.orderD[0], cfg.betaD[0],
				CtrlRateHz*CtrlRateDiv );
			((ESO_AngularRate*)CtrlM_ESO[2])->init( 
				(cfg.Mode[0] & ATTCTRL_MODE_Y_LEARN_DIS_BIT) ? false : false,
				cfg.order2[0], 1.0/(CtrlRateHz*CtrlRateDiv), cfg.b[4], cfg.beta2[0], cfg.beta2[0], 
				cfg.orderD2[0], cfg.betaD2[0],
				CtrlRateHz*CtrlRateDiv );
	...
	/*三轴*/
		static void TriRotor_Init()
		{
			...
			((ESO_AngularRate*)CtrlM_ESO[2])->init( 
				false,
				cfg.order[0], cfg.T2[0], cfg.b[4], cfg.beta2[0], cfg.beta2[0], 
				cfg.orderD2[0], cfg.betaD2[0],
				CtrlRateHz*CtrlRateDiv );
	...
	/*直升机*/
		...
		{
			//初始化姿态ESO
			((ESO_AngularRateHeli*)CtrlM_ESO[0])->init( cfg.order[0], cfg.T[0], cfg.b[0]*Heli_RPb_scale, cfg.beta[0], 
				cfg.orderD[0], cfg.betaD[0],
				CtrlRateHz*CtrlRateDiv );
			((ESO_AngularRateHeli*)CtrlM_ESO[1])->init( cfg.order[0], cfg.T[0], cfg.b[2]*Heli_RPb_scale, cfg.beta[0], 
				cfg.orderD[0], cfg.betaD[0],
				CtrlRateHz*CtrlRateDiv );
			((ESO_AngularRate*)CtrlM_ESO[2])->init( 
				false,
				cfg.order2[0], cfg.T2[0], cfg.b[4], cfg.beta2[0], cfg.beta2[0], 
				cfg.orderD2[0], cfg.betaD2[0],
				CtrlRateHz*CtrlRateDiv );
		}
...
//			ESOHeli[1].init( cfg.T[0], cfg.b[2]*Heli_RPb_scale, cfg.beta[0], CtrlRateHz*CtrlRateDiv );
			((ESO_AngularRate*)CtrlM_ESO[0])->init( 
				false,
				cfg.order[0], cfg.T2[0], cfg.b[0], cfg.beta[0], cfg.beta[0], 
				cfg.orderD[0], cfg.betaD[0],
				CtrlRateHz*CtrlRateDiv );
			((ESO_AngularRate*)CtrlM_ESO[1])->init( 
				false,
				cfg.order[0], cfg.T2[0], cfg.b[2], cfg.beta[0], cfg.beta[0], 
				cfg.orderD[0], cfg.betaD[0],
				CtrlRateHz*CtrlRateDiv );
			((ESO_AngularRate*)CtrlM_ESO[2])->init( 
				false,
	...
	if( pre_inFlightRP || (CtrlM.RP_NoDisturbanceOG==false) )
	{
		switch( CtrlM.RP_ctrl_type )
		{
			...
			{	//控制推力导数
				...
				outRoll = ( 
						angular_acceleration_filted.x-disturbanceCtrl[0]
						+ CtrlM_ESO[0]->get_T() * ( angular_acceleration_error.x * P3.x + Ta1.x /*- disturbanceD[0]*/ ) 
					)/CtrlM_ESO[0]->get_b();
				outPitch = ( 
						angular_acceleration_filted.y-disturbanceCtrl[1]
						+ CtrlM_ESO[1]->get_T() * ( angular_acceleration_error.y * P3.y + Ta1.y /*- disturbanceD[1]*/ ) 
					)/CtrlM_ESO[1]->get_b();
				break;
			}
			case 1:
			...
			{	//控制角速度导数 (直升机姿态控制2)
				...
				outRoll = ( 
						angular_rate_ESO.x-disturbanceCtrl[0]
						+ CtrlM_ESO[0]->get_T() * ( angular_rate_error.x * P2.x + Tv1.x - disturbanceD[0] ) 
					)/CtrlM_ESO[0]->get_b();
				outPitch = ( 
						angular_rate_ESO.y-disturbanceCtrl[1]
						+ CtrlM_ESO[1]->get_T() * ( angular_rate_error.y * P2.y + Tv1.y - disturbanceD[1] ) 
					)/CtrlM_ESO[1]->get_b();
				break;
			}
		}
	...
	if( pre_inFlightY || (CtrlM.RP_NoDisturbanceOG==false) )
	{
		switch( CtrlM.Y_ctrl_type )
		{
			...
			{	//控制推力导数
				...
				outYaw = ( 
						angular_acceleration_filted.z-disturbanceCtrl
						+ CtrlM_ESO[2]->get_T() * ( angular_acceleration_error.z * P3.z + Ta1.z - disturbanceD[2] ) 
					)/CtrlM_ESO[2]->get_b();
...
void init_Ctrl_Attitude()
{
	...
	cfg.P1[0] = 4;	cfg.P1[2] = 4;	cfg.P1[4] = 2;
	cfg.P2[0] = 10;	cfg.P2[2] = 10;	cfg.P2[4] = 5;
	cfg.P3[0] = 36;	cfg.P3[2] = 36;	cfg.P3[4] = 25;
	cfg.order[0] = 2;
	cfg.order2[0] = 2;
	cfg.orderD[0] = 2;
	cfg.orderD2[0] = 2;
