...
bool is_insideFence(const vector3<double> &pos, const vector3<double> &targetVel, double *distance, FRS *fenceRs)
{
	...
	// 距离最远的inclusion围栏
	double max_InFenceDistance = -1;
	int16_t max_InFenceDistanceInd = -1;
	// 禁飞区状态 -1-不存在禁飞区 0-在禁飞区外 1-在禁飞区内但速度指向区域外 2-在禁飞区内
	int8_t exFenceState = -1;
	// 距离最近的exclusion围栏
	double min_ExFenceDistance = -1;
	double max_NotExFenceDistance = -1;
	// 速度指向围栏内时，离围栏距离
						  ...
						  double *distance)
{
	...
	double x = circleO_x - pos.x;
	double y = circleO_y - pos.y;
	// 投影圆心到速度方向
	vector2<double> CPos;
	CPos.x = ENU2BodyHeading_x(x, y, sinVel, cosVel);
	CPos.y = ENU2BodyHeading_y(x, y, sinVel, cosVel);
