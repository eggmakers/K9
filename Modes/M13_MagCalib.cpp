...
ModeResult M13_MagCalib::main_func(void *param1, uint32_t param2)
{
	...
	int8_t st_mag_ind = -1;
	double calibrate_complete_condition;
	for (uint8_t i = 0; i < IMU_Sensors_Count; ++i)
	{
		...
		if (GetMagnetometer(i, &sensor, 1))
		{
			...
			if (st_mag_ind < 0)
			{
				...
				max_x = min_x = sensor.data_raw.x;
				max_y = min_y = sensor.data_raw.y;
				max_z = min_z = sensor.data_raw.z;
				calibrate_complete_condition = 0.3 / sensor.sensitivity;
			}
		}
		else
...
#define h 0.02
	...
	while (1)
	{
		...
		if (msg_available)
		{
			...
			else if (msg.cmd == 241 && msg.params[1] == 0)
			{
		...
		// 计算进度
		uint8_t sc[3];
		sc[0] = constrain((float)(max_x - min_x) * 0xf / (float)calibrate_complete_condition, 0.0f, 15.0f);
		param2 |= (sc[0] << (8 + 3 * 4));
		sc[1] = constrain((float)(max_y - min_y) * 0xf / (float)calibrate_complete_condition, 0.0f, 15.0f);
		param2 |= (sc[1] << (8 + 4 * 4));
		sc[2] = constrain((float)(max_z - min_z) * 0xf / (float)calibrate_complete_condition, 0.0f, 15.0f);
		param2 |= (sc[2] << (8 + 5 * 4));
		// 计算总进度
		float progress = (float)(sc[0] + sc[1] + sc[2]) / 16 / 3 * 100;
		...
		if (rotate_start_time.is_valid() && rotate_start_time.get_pass_time() > 0.2f)
		{
			...
			IMU_Sensor st_magnetometer;
			GetMagnetometer(st_mag_ind, &st_magnetometer, 1);
#define MagMMFilter 0.35
			if (st_magnetometer.data_raw.x > max_x)
				max_x += MagMMFilter * h * (st_magnetometer.data_raw.x - max_x);
			else if (st_magnetometer.data_raw.x < min_x)
				min_x += MagMMFilter * h * (st_magnetometer.data_raw.x - min_x);
			if (st_magnetometer.data_raw.y > max_y)
				max_y += MagMMFilter * h * (st_magnetometer.data_raw.y - max_y);
			else if (st_magnetometer.data_raw.y < min_y)
				min_y += MagMMFilter * h * (st_magnetometer.data_raw.y - min_y);
			if (st_magnetometer.data_raw.z > max_z)
				max_z += MagMMFilter * h * (st_magnetometer.data_raw.z - max_z);
			else if (st_magnetometer.data_raw.z < min_z)
				min_z += MagMMFilter * h * (st_magnetometer.data_raw.z - min_z);
			...
			for (uint8_t i = 0; i < IMU_Sensors_Count; ++i)
			{
				...
				{
					double x = sensor.data_raw.x;
					double y = sensor.data_raw.y;
					double z = sensor.data_raw.z;
					double x2 = x * x;
					double x3 = x2 * x;
					double x4 = x3 * x;
					double y2 = y * y;
					...
					info.sum_x2y[i] += x2y;
					info.sum_xy2[i] += xy2;
					info.sum_x2y2[i] += x2y2;
					info.sum_xz[i] += xz;
					info.sum_x2z[i] += x2z;
					info.sum_xz2[i] += xz2;
					info.sum_x2z2[i] += x2z2;
					info.sum_yz[i] += yz;
					info.sum_y2z[i] += y2z;
					info.sum_yz2[i] += yz2;
					info.sum_y2z2[i] += y2z2;
				}
				else
					sensor_present[i] = false;
			}
			...
			setLedManualCtrl(0, 0, progress, false, 0);
			if (
		...
		{ // 向每个端口发送ACK
			...
			// 计算独立进度
			uint32_t param2 = 2;
			if (fabs(angular_rate.x) > 0.6)
				param2 |= (1 << (8 + 0 * 4));
			if (fabs(angular_rate.y) > 0.6)
				param2 |= (1 << (8 + 1 * 4));
			if (fabs(angular_rate.z) > 0.6)
				param2 |= (1 << (8 + 2 * 4));
		...
		if (sensor_present[i] && GetMagnetometer(i, &sensor, 1))
		{
			...
			while (1)
			{
				...
				if (counter > 1000 && (dx_matrix[0] * dx_matrix[0] + dx_matrix[1] * dx_matrix[1] + dx_matrix[2] * dx_matrix[2] + dx_matrix[3] * dx_matrix[3] + dx_matrix[4] * dx_matrix[4]) < 0.01)
				{
					...
					cfg.offset[0] = offset.x;
					cfg.offset[1] = offset.y;
					cfg.offset[2] = offset.z;
					cfg.scale[0] = scale.x;
					cfg.scale[1] = scale.y;
					cfg.scale[2] = scale.z;
					if (res == PR_OK)
...
static void get_F_dF(uint8_t index, double es[6], double F_matrix[6], double dF_matrix[36], const MagCalibInfo *info)
{
	...
	dF_matrix[0] = Aa_2 * info->sum_x[index] - A_2 * info->sum_x2[index];
	dF_matrix[1] = Bb_2 * info->sum_x[index] - B_2 * info->sum_xy[index];
	dF_matrix[2] = 2 * (es[2] * info->sum_x[index] - info->sum_xz[index]);
	dF_matrix[3] = a2 * info->sum_x[index] - a_2 * info->sum_x2[index] + info->sum_x3[index];
	dF_matrix[4] = b2 * info->sum_x[index] - b_2 * info->sum_xy[index] + info->sum_xy2[index];
	dF_matrix[5] = -info->sum_x[index];
	dF_matrix[6] = Aa_2 * info->sum_y[index] - A_2 * info->sum_xy[index];
	dF_matrix[7] = Bb_2 * info->sum_y[index] - B_2 * info->sum_y2[index];
	dF_matrix[8] = 2 * (es[2] * info->sum_y[index] - info->sum_yz[index]);
	dF_matrix[9] = a2 * info->sum_y[index] - a_2 * info->sum_xy[index] + info->sum_x2y[index];
	dF_matrix[10] = b2 * info->sum_y[index] - b_2 * info->sum_y2[index] + info->sum_y3[index];
	dF_matrix[11] = -info->sum_y[index];
	dF_matrix[12] = Aa_2 * info->sum_z[index] - A_2 * info->sum_xz[index];
	dF_matrix[13] = Bb_2 * info->sum_z[index] - B_2 * info->sum_yz[index];
	dF_matrix[14] = 2 * (es[2] * info->sum_z[index] - info->sum_z2[index]);
	dF_matrix[15] = a2 * info->sum_z[index] - a_2 * info->sum_xz[index] + info->sum_x2z[index];
	dF_matrix[16] = b2 * info->sum_z[index] - b_2 * info->sum_yz[index] + info->sum_y2z[index];
	dF_matrix[17] = -info->sum_z[index];
	dF_matrix[18] = Aa_2 * info->sum_x2[index] - A_2 * info->sum_x3[index];
	dF_matrix[19] = Bb_2 * info->sum_x2[index] - B_2 * info->sum_x2y[index];
	dF_matrix[20] = 2 * (es[2] * info->sum_x2[index] - info->sum_x2z[index]);
	dF_matrix[21] = a2 * info->sum_x2[index] - a_2 * info->sum_x3[index] + info->sum_x4[index];
	dF_matrix[22] = b2 * info->sum_x2[index] - b_2 * info->sum_x2y[index] + info->sum_x2y2[index];
	dF_matrix[23] = -info->sum_x2[index];
	dF_matrix[24] = Aa_2 * info->sum_y2[index] - A_2 * info->sum_xy2[index];
	dF_matrix[25] = Bb_2 * info->sum_y2[index] - B_2 * info->sum_y3[index];
	dF_matrix[26] = 2 * (es[2] * info->sum_y2[index] - info->sum_y2z[index]);
	dF_matrix[27] = a2 * info->sum_y2[index] - a_2 * info->sum_xy2[index] + info->sum_x2y2[index];
	dF_matrix[28] = b2 * info->sum_y2[index] - b_2 * info->sum_y3[index] + info->sum_y4[index];
	dF_matrix[29] = -info->sum_y2[index];
	dF_matrix[30] = Aa_2 * info->n - A_2 * info->sum_x[index];
	dF_matrix[31] = Bb_2 * info->n - B_2 * info->sum_y[index];
	dF_matrix[32] = 2 * (es[2] * info->n - info->sum_z[index]);
	dF_matrix[33] = a2 * info->n - a_2 * info->sum_x[index] + info->sum_x2[index];
	dF_matrix[34] = b2 * info->n - b_2 * info->sum_y[index] + info->sum_y2[index];
	dF_matrix[35] = -info->n;
}