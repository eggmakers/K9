...
ModeResult M12_AccCalib::main_func(void *param1, uint32_t param2)
{
	...
	while (1)
	{
		...
		{ // 向每个端口发送ACK
			...
			// 计算总进度
			float counter = Calibration_Counter;
			if (counter < 0)
				counter = 0;
			float progress = ((float)calibrated_surface_count / 6 + (float)counter / Calibration_Time / 6) * 100;
			// 计算独立进度
			uint32_t param2 = 5;
			for (uint8_t i = 0; i < 6; ++i)
			{
				if (calibrated_surface[i])
					param2 |= (0xf << (8 + i * 4));
				else if (Current_Surface == i && Calibration_Counter > 0)
				{
					uint8_t sc = 0xf * ((float)Calibration_Counter / Calibration_Time);
					param2 |= (sc << (8 + i * 4));
				}
			}
		...
		if (calibrated_surface_count < 6)
		{
			...
			if (acc_f32.x > 600.0f)
				Current_Surface = 0;
			else if (acc_f32.x < -600.0f)
				Current_Surface = 1;
			else if (acc_f32.y > 600.0f)
				Current_Surface = 2;
			else if (acc_f32.y < -600.0f)
				Current_Surface = 3;
			else if (acc_f32.z > 600.0f)
				Current_Surface = 4;
			else if (acc_f32.z < -600.0f)
				Current_Surface = 5;
			...
			else
			{
				...
				/*判断飞控静止*/
				if (acc_filted.x > Calibration_Acc_Max.x)
					Calibration_Acc_Max.x = acc_filted.x;
				else if (acc_filted.x < Calibration_Acc_Min.x)
					Calibration_Acc_Min.x = acc_filted.x;
				if (acc_filted.y > Calibration_Acc_Max.y)
					Calibration_Acc_Max.y = acc_filted.y;
				else if (acc_filted.y < Calibration_Acc_Min.y)
					Calibration_Acc_Min.y = acc_filted.y;
				if (acc_filted.z > Calibration_Acc_Max.z)
					Calibration_Acc_Max.z = acc_filted.z;
				else if (acc_filted.z < Calibration_Acc_Min.z)
					Calibration_Acc_Min.z = acc_filted.z;

				if (gyro_filted.x > Calibration_Gyro_Max.x)
					Calibration_Gyro_Max.x = gyro_filted.x;
				else if (gyro_filted.x < Calibration_Gyro_Min.x)
					Calibration_Gyro_Min.x = gyro_filted.x;
				if (gyro_filted.y > Calibration_Gyro_Max.y)
					Calibration_Gyro_Max.y = gyro_filted.y;
				else if (gyro_filted.y < Calibration_Gyro_Min.y)
					Calibration_Gyro_Min.y = gyro_filted.y;
				if (gyro_filted.z > Calibration_Gyro_Max.z)
					Calibration_Gyro_Max.z = gyro_filted.z;
				else if (gyro_filted.z < Calibration_Gyro_Min.z)
					Calibration_Gyro_Min.z = gyro_filted.z;

				double acc_fluctuation_range;
				double gyro_fluctuation_range;
		...
		if (GetAccelerometer(i, &sensor, 1) == true)
		{
			...
			while (1)
			{
				...
				if (Calibration_Counter > 2000 &&
					(dx_matrix[0] * dx_matrix[0] + dx_matrix[1] * dx_matrix[1] + dx_matrix[2] * dx_matrix[2] + dx_matrix[3] * dx_matrix[3] + dx_matrix[4] * dx_matrix[4] + dx_matrix[5] * dx_matrix[5]) < 0.01)
				{ // 精度达到 迭代结束
					...
					cfg.offset[1] = es[1];
					cfg.offset[2] = es[2];
					cfg.scale[0] = Gravity_DP / es[3];
					cfg.scale[1] = Gravity_DP / es[4];
					cfg.scale[2] = Gravity_DP / es[5];
					if (res == PR_OK)
...
static void get_F_dF(double F_matrix[6], double dF_matrix[36], double *es, double *p)
{
	...
	double A2 = es[3] * 2.0;
	double B2 = es[4] * 2.0;
	dF_matrix[00] = A2 * a_x1;
	dF_matrix[01] = B2 * b_y1;
	dF_matrix[02] = 2 * c_z1;
	dF_matrix[03] = a_x1 * a_x1;
	dF_matrix[04] = b_y1 * b_y1;
	dF_matrix[05] = -1;
	dF_matrix[06] = A2 * a_x2;
	dF_matrix[07] = B2 * b_y2;
	dF_matrix[8] = 2 * c_z2;
	dF_matrix[9] = a_x2 * a_x2;
	dF_matrix[10] = b_y2 * b_y2;
	dF_matrix[11] = -1;
	dF_matrix[12] = A2 * a_x3;
	dF_matrix[13] = B2 * b_y3;
	dF_matrix[14] = 2 * c_z3;
	dF_matrix[15] = a_x3 * a_x3;
	dF_matrix[16] = b_y3 * b_y3;
	dF_matrix[17] = -1;
	dF_matrix[18] = A2 * a_x4;
	dF_matrix[19] = B2 * b_y4;
	dF_matrix[20] = 2 * c_z4;
	dF_matrix[21] = a_x4 * a_x4;
	dF_matrix[22] = b_y4 * b_y4;
	dF_matrix[23] = -1;
	dF_matrix[24] = A2 * a_x5;
	dF_matrix[25] = B2 * b_y5;
	dF_matrix[26] = 2 * c_z5;
	dF_matrix[27] = a_x5 * a_x5;
	dF_matrix[28] = b_y5 * b_y5;
	dF_matrix[29] = -1;
	dF_matrix[30] = A2 * a_x6;
	dF_matrix[31] = B2 * b_y6;
	dF_matrix[32] = 2 * c_z6;
	dF_matrix[33] = a_x6 * a_x6;
	dF_matrix[34] = b_y6 * b_y6;
	dF_matrix[35] = -1;
}
