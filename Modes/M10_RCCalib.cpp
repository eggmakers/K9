...
ModeResult M10_RCCalib::main_func(void *param1, uint32_t param2)
{
	...
	SName rc_name;
	uint8_t rc_channels;
	if (getReceiver(&rc, &rc_name, 0.1) == false || rc.preCalibrated)
	{
		sendLedSignal(LEDSignal_Err1);
		os_delay(1.0);
		return MR_Err;
	}
	rc_channels = rc.raw_available_channels;

	set_mav_state(MAV_STATE_CALIBRATING);

	uint8_t send_ack_counter = 0;
	bool Calibration_Started = true;
...
#define reset_constant_check                                \
	...
	while (1)
	{
		...
		if (msg_available)
		{
			...
			else if (msg.cmd == 241 && msg.params[3] == 0)
			{
		...
		if (new_rc_name != rc_name || rc.raw_available_channels != rc_channels)
		{
			sendLedSignal(LEDSignal_Err1);
			os_delay(1.0);
			return MR_Err;
		...
		case 4:
		{
			...
			if (Calibration_Stage2 >= avg_time)
			{
				...
				for (unsigned char i = 0; i < rc_channels; ++i)
				{
