...
/*InflightCmd183_MAV_CMD_DO_SET_SERVO
...
*/
static bool InflightCmd183_DO_SET_SERVO(uint16_t cmd, double params[])
{
	if (params[0] > 0)
		setAuxPWM(params[1], params[0] - 1);
	else
		return false;
	return true;
}
...
/*InflightCmd2000_MAV_CMD_IMAGE_START_CAPTURE
...
*/
static bool InflightCmd2000_MAV_CMD_IMAGE_START_CAPTURE(uint16_t cmd, double params[])
{
	CamTriggDist = 0;
	if (params[2] == 1)
		return AuxCamTakePhoto();
	return false;
}
...
static bool (*const InflightCmdProcess[])(uint16_t cmd, double params[]) =
	{
		...
		/*181-*/ 0,
		/*182-*/ 0,
		/*183-*/ InflightCmd183_DO_SET_SERVO,
		/*184-*/ 0,
		/*185-*/ 0,
		/*186-*/ 0,
		/*187-*/ 0,
...
bool Process_InflightCmd(uint16_t cmd, double params[])
{
	...
	{
		if (cmd == MAV_CMD_IMAGE_START_CAPTURE)
			return InflightCmd2000_MAV_CMD_IMAGE_START_CAPTURE(cmd, params);
		return false;
	}
