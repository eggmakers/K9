...
/*内部存储*/
	SS_RESULT lock_InternalStorage(double TIMEOUT=-1);
	...
	/*配置存储*/
		/*
			...
			TIMEOUT：线程同步超时时间
		*/
		SS_RESULT InternalStorage_TruncateFile( const char* group_name, const char* name, uint32_t length, double TIMEOUT = -1 );
		...
		/*
			...
			TIMEOUT：线程同步超时时间
		*/
		SS_RESULT InternalStorage_WriteFile( const char* group_name, const char* name, const void* content, uint32_t offset, uint32_t length, double TIMEOUT = -1 );
		...
		*/
		SS_RESULT InternalStorage_ReadFile( const char* group_name, const char* name, void* content , uint32_t* length, 
				uint32_t read_offset=0, int32_t read_length=-1, double TIMEOUT = -1 );
	/*配置存储*/
...
/*外部存储*/
	...
	/*Log数据*/
		...
		*/
		...
		enum IMUType
		{
			IMUType_Accelerometer = 1,
			IMUType_Gyroscope = 2,
			IMUType_Magnetometer = 3,
			IMUType_DFAntenna = 4,
		};
	
		bool SDLog_Msg_SysState( double SyncTIMEOUT = -1 );
	...
	/*Ubx Log*/
		bool SDLog_Ubx( const char* txt, uint16_t length, double SyncTIMEOUT = -1 );
	/*Ubx Log*/		
		
	//获取SD卡总空间GB
	float getSdTotalSizeGB();
	//获取SD卡剩余空间GB
	float getSdFreeSizeGB();
	
	//清理SD卡
	//monthBefore:清理n月前的记录 <0时清理所有记录
