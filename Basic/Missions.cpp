...
bool clearMissions( double TIMEOUT )
{
	...
	UpdateParamGroup( "RestoreWpInf", (uint64_t*)&initial_RestoreWpInf, 0, sizeof(RestoreWpInf)/8 );
	return true;
}
...
bool addMission( MissionInf wp_inf, bool st, double TIMEOUT )
{
	...
	if( Lock_Missions(TIMEOUT) )
	{
		if( UploadingMissionsCount >= MaxMissions )
		{
			UnLock_Missions();
			return false;
		}
		...
		PR_RESULT res = WriteVolatileParamGroup( "Missions", &wp_inf, UploadingMissionsCount, 1, st );
		if(st)
		{
			++MissionsCount;
...
/*
	...
	false:添加失败（航点过多或航点信息错误或参数表错误）
*/
bool editMission( uint16_t ind, const MissionInf* wp_inf, double TIMEOUT )
{
	if( wp_inf->cmd == 0 )
		return false;
	...
	if( Lock_Missions(TIMEOUT) )
	{
		if( ind >= UploadingMissionsCount )
		{
			UnLock_Missions();
			return false;
		}
		bool res = WriteVolatileParamGroup( "Missions", &wp_inf, ind, 1, true );
...
*/
bool editCurrentMission( MissionInf* wp_inf, double TIMEOUT )
{
	if( MissionsCount == 0 )
		return false;
	...
	if( Lock_Missions(TIMEOUT) )
	{
		if( CurrentMission >= UploadingMissionsCount )
		{
			UnLock_Missions();
			return false;
		}
		bool res = WriteVolatileParamGroup( "Missions", &wp_inf, CurrentMission, 1, true );
...
{	
	...
	if( (MFunc_cfg.configs[0] & MCfg_RstWp0_Bit) == 0 )
	{
		//初始化CurrentMissionInd
		RestoreWpInf restoreWpInf;
		ReadParamGroup( "RestoreWpInf", (uint64_t*)&restoreWpInf, 0 );
		
		if( restoreWpInf.CurrentWp[0] < missions_count )
			setCurrentMission(restoreWpInf.CurrentWp[0]);
	}
	else
		setCurrentMission(0);
}