...
/*RTC时间获取*/
  ...
  //UTC转本地时间
	...
	extern "C" uint32_t LocalTime2Unix( const RTC_TimeStruct* rtc_local, int8_t TimeZone )
	{
		struct tm time;
    time.tm_year = rtc_local->Year - 1900;	/* 年份修正 */
    time.tm_mon = rtc_local->Month - 1;		/* 月份修正 */
    time.tm_mday = rtc_local->Date;
    time.tm_hour = rtc_local->Hours;
    time.tm_min = rtc_local->Minutes;
    time.tm_sec = rtc_local->Seconds;
		
		time_t timestamp = mktime(&time) - (uint32_t)TimeZone*60*60;
		return timestamp;
	}
/*RTC时间获取*/
