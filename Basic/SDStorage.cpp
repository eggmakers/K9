...
bool SDLog_Msg_PosSensor(uint8_t ind, Position_Sensor sensor, double SyncTIMEOUT)
{
	...
	uint8_t dataType = sensor.data.sensor_DataType;
	if (dataType > 100)
		dataType -= 100;
	if (dataType < 8)
	{ // s传感器
...
reload_SD:
	...
	if (f_mount(&SDFatFS, (TCHAR const *)SDPath, 1) == FR_OK)
	{
		...
		static FIL SDFile;
		static FIL Txt1File;
		bool Txt1FileCreated = false;
		static FIL UbxFile;
		bool UbxFileCreated = false;
		// 文件操作状态
		FRESULT fres;
		// 文件（夹）状态
		FILINFO finfo;
		...
		// 写描述字符
		sprintf(buf, "ACFlyK9 %d%02d%02d%02d%02d", RTC_Time.Year, RTC_Time.Month, RTC_Time.Date, RTC_Time.Hours, RTC_Time.Minutes);
