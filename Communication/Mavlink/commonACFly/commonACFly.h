/** @file
 *  @brief MAVLink comm protocol generated from commonACFly.xml
 *  @see http://mavlink.org
 */
#pragma once
#ifndef MAVLINK_COMMONACFLY_H
#define MAVLINK_COMMONACFLY_H

#ifndef MAVLINK_H
#error Wrong include order: MAVLINK_COMMONACFLY.H MUST NOT BE DIRECTLY USED. Include mavlink.h from the same directory instead or set ALL AND EVERY defines from MAVLINK.H manually accordingly, including the #define MAVLINK_H call.
#endif
...
#ifndef HAVE_ENUM_FTP_PROTOCOL_TYPE
#define HAVE_ENUM_FTP_PROTOCOL_TYPE
   enum OpCode_t : uint8_t
   {
      opCodeNone = 0,         ///< ignored, always acked
      opCodeTerminateSession, ///< Terminates open Read session
      opCodeResetSessions,    ///< Terminates all open Read sessions
      opCodeListDirectory,    ///< List files in <path> from <offset>
      opCodeOpenFileRO,       ///< Opens file at <path> for reading, returns <session>
      opCodeReadFile,         ///< Reads <size> bytes from <offset> in <session>
      opCodeCreateFile,       ///< Creates file at <path> for writing, returns <session>
      opCodeWriteFile,        ///< Writes <size> bytes to <offset> in <session>
      opCodeRemoveFile,       ///< Remove file at <path>
      opCodeCreateDirectory,  ///< Creates directory at <path>
      opCodeRemoveDirectory,  ///< Removes Directory at <path>, must be empty
      opCodeOpenFileWO,       ///< Opens file at <path> for writing, returns <session>
      opCodeTruncateFile,     ///< Truncate file at <path> to <offset> length
      opCodeRename,           ///< Rename <path1> to <path2>
      opCodeCalcFileCRC32,    ///< Calculate CRC32 for file at <path>
      opCodeBurstReadFile,    ///< Burst download session file

      opCodeRspAck = 128, ///< Ack response
      opCodeRspNak,       ///< Nak response
   };
   enum OpErrorCode : uint8_t
   {
      opCodeErrNone = 0,
      opCodeErrFail,                ///< Unknown failure
      opCodeErrFailErrno,           ///< Command failed, errno sent back in PayloadHeader.data[1]
      opCodeErrInvalidDataSize,     ///< PayloadHeader.size is invalid
      opCodeErrInvalidSession,      ///< Session is not currently open
      opCodeErrNoSessionsAvailable, ///< All available Sessions in use
      opCodeErrEOF,                 ///< Offset past end of file for List and Read commands
      opCodeErrUnknownCommand,      ///< Unknown command opcode
      opCodeErrFailFileExists,      ///< File/directory exists already
      opCodeErrFailFileProtected,   ///< File/directory is write protected
      opCodeErrFileNotFound         ///< File/directory not found
   };
#endif
...
#define HAVE_ENUM_MAV_FRAME
   typedef enum MAV_FRAME
   {
      ...
      MAV_FRAME_BODY_FLU = 13,               /* MAV_FRAME_BODY_FLU - Body fixed frame of reference, Z-up (x: Forward, y: Left, z: Up). | */
...
#define HAVE_ENUM_MAV_CMD
   ...
   {
      MAV_CMD_SEND_WGA = 10,                            /* request vehicle to send WGA code. |WGA index| Empty| Empty| Empty| Empty| Empty| Empty|  */
      MAV_CMD_WRITE_WGA = 11,                           /* request vehicle to write WGA password. |password1| password2| password3| password4| Empty| Empty| Empty|  */
      MAV_CMD_REQUEST_ACFLY_POSSENSOR_INFO_STREAM = 14, /* request vehicle to write WGA password. |possensor ind| rate| 0:new request 1:cancel others(this only)| Empty| Empty| Empty| Empty|  */
      ...
      MAV_CMD_DO_GIMBAL_MANAGER_LASER = 1002,
      MAV_CMD_DO_GIMBAL_MANAGER_THERMAL = 1003,
      MAV_CMD_DO_GIMBAL_MANAGER_CENTER = 1004,
      MAV_CMD_DO_GIMBAL_MANAGER_UP = 1005,
      MAV_CMD_DO_GIMBAL_MANAGER_DOWN = 1006,
      MAV_CMD_DO_GIMBAL_START_TARGET_DETECTION = 1007,
      MAV_CMD_DO_GIMBAL_STOP_TARGET_DETECTION = 1008,
      MAV_CMD_DO_GIMBAL_ENABLE_OSD = 1009,
      MAV_CMD_DO_GIMBAL_DISABLE_OSD = 1010,
...
#define HAVE_ENUM_MAV_DO_REPOSITION_FLAGS
   typedef enum MAV_DO_REPOSITION_FLAGS
   {
      ...
      MAV_DO_REPOSITION_FLAGS_ALTITUDE_MODE = 2, /* The aircraft should change altitude first and then flight to the coordinate | */
      MAV_DO_REPOSITION_FLAGS_ENUM_END = 3,      /*  | */
   } MAV_DO_REPOSITION_FLAGS;
#endif

/** @brief These flags indicate status such as data validity of each data source. Set = data valid */
#ifndef HAVE_ENUM_ACFLY_IMU_FLAGS
#define HAVE_ENUM_ACFLY_IMU_FLAGS
   typedef enum ACFLY_IMU_FLAGS
   {
      ACCEL0 = 1,                      /*  | */
      ACCEL1 = 2,                      /*  | */
      ACCEL2 = 4,                      /*  | */
      ...
      COMPASS1 = 512,                  /*  | */
      COMPASS2 = 1024,                 /*  | */
      ACFLY_IMU_FLAGS_ENUM_END = 1025, /*  | */
   } ACFLY_IMU_FLAGS;
#endif

/** @brief These flags indicate status such as data validity of each data source. Set = data valid */
#ifndef HAVE_ENUM_ACFLY_POSTYPE_FLAGS
#define HAVE_ENUM_ACFLY_POSTYPE_FLAGS
   typedef enum ACFLY_POSTYPE_FLAGS
   {
      GlobalPositioning = 0,            /*  | */
      RelativePositioning = 1,          /*  | */
      RangePositioning = 2,             /*  | */
      ACFLY_POSTYPE_FLAGS_ENUM_END = 3, /*  | */
   } ACFLY_POSTYPE_FLAGS;
#endif

/** @brief These flags indicate status such as data validity of each data source. Set = data valid */
#ifndef HAVE_ENUM_ACFLY_POSDATATYPE_FLAGS
#define HAVE_ENUM_ACFLY_POSDATATYPE_FLAGS
   typedef enum ACFLY_POSDATATYPE_FLAGS
   {
      s_xy = 0,                              /*  | */
      s_z = 1,                               /*  | */
      s_xyz = 2,                             /*  | */
      ...
      sv_z = 17,                             /*  | */
      sv_xyz = 18,                           /*  | */
      ACFLY_POSDATATYPE_FLAGS_ENUM_END = 19, /*  | */
   } ACFLY_POSDATATYPE_FLAGS;
#endif
...
#define HAVE_ENUM_CAMERA_CAP_FLAGS
   typedef enum CAMERA_CAP_FLAGS
   {
      ...
      CAMERA_CAP_FLAGS_HAS_LASER = (1 << 12),
      CAMERA_CAP_FLAGS_HAS_THERMAL = (1 << 13),
      CAMERA_CAP_FLAGS_HAS_DOUBLE_VIDEO = (1 << 14),
      CAMERA_CAP_FLAGS_HAS_GIMBAL_PITCH = (1 << 15),
      CAMERA_CAP_FLAGS_HAS_GIMBAL_YAW = (1 << 16),
      CAMERA_CAP_FLAGS_HAS_GIMBAL_UP = (1 << 17),
      CAMERA_CAP_FLAGS_HAS_GIMBAL_DOWN = (1 << 18),
      CAMERA_CAP_FLAGS_HAS_GIMBAL_CENTER = (1 << 19),
      CAMERA_CAP_FLAGS_HAS_LED = (1 << 20),
      CAMERA_CAP_FLAGS_HAS_FLIP = (1 << 21),
      CAMERA_CAP_FLAGS_HAS_SWITCH = (1 << 22),
      CAMERA_CAP_FLAGS_HAS_CALIBRATION = (1 << 23),
      CAMERA_CAP_FLAGS_HAS_ACK = (1 << 24),
      CAMERA_CAP_FLAGS_SHOW_UI_BUTTON = (1 << 25),
      CAMERA_CAP_FLAGS_GIMBAL_CTRL_SEND_WHEN_CHANGED = (1 << 26),
      CAMERA_CAP_FLAGS_TARGET_DETECTION = (1 << 27),
      CAMERA_CAP_FLAGS_HAS_OSD = (1 << 28),
      CAMERA_CAP_FLAGS_SHOW_CAMERA_AREA = (1 << 29),
      CAMERA_CAP_FLAGS_TARGET_SELECTION_RECTANGLE = (1 << 30),
      CAMERA_CAP_FLAGS_ENUM_END = (1 << 31) /*  | */
   } CAMERA_CAP_FLAGS;
#endif
...
/** @brief MAV_MISSION_CURRENT_FLAG. */
#ifndef HAVE_ENUM_MAV_MISSION_CURRENT_FLAG
#define HAVE_ENUM_MAV_MISSION_CURRENT_FLAG
   typedef enum MAV_MISSION_CURRENT_FLAG
   {
      MAV_MISSION_CURRENT_IS_IN_MISSION_FLIGHT = 1,     /* 1: vehicle is inFlight | */
      MAV_MISSION_CURRENT_IS_RESUME_WAYPOINT_EXIST = 2, /* 1: resume waypoint info exist | */
      MAV_MISSION_CURRENT_FLAG_ENUM_END = 3,            /*  | */
   } MAV_MISSION_CURRENT_FLAG;
#endif
...
#include "./mavlink_msg_acflypossensor_info.h"
#include "./mavlink_msg_acfly_regeisterpossensor.h"
#include "./mavlink_msg_acfly_updatepossensor.h"
